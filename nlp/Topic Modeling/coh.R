...
library(ggplot2)
library(Matrix)
dtm_matrix <- as.matrix(dtm)  # DTM을 일반 행렬로 변환
dtm_sparse <- Matrix(dtm_matrix, sparse = TRUE)  # 희소 행렬로 변환

# 토픽-단어 분포 (phi) 가져오기
topic_word_probs <- lda_model@beta  # log-scale 값
#---------------------------------------------------------------------------#
# Semantic Coherence 계산 함수 정의
CalcCoherence_FirstFormula <- function(phi, dtm) {
  if (!is(dtm, "CsparseMatrix")) {
    stop("dtm must be a sparse matrix (CsparseMatrix)")
  }
  
  coherence <- numeric(nrow(phi))  # 각 토픽에 대한 Coherence 저장
  for (k in seq_len(nrow(phi))) {  # 각 토픽별로 계산
    top_words <- order(phi[k, ], decreasing = TRUE)[1:15]  # 상위 20개 단어 선택
    total <- 0
    for (m in 2:length(top_words)) {
      for (l in 1:(m - 1)) {
        word_m <- top_words[m]
        word_l <- top_words[l]
        
        # 단어 m과 l의 동시 등장 문서 수 계산
        co_docs <- sum(dtm[, word_m] > 0 & dtm[, word_l] > 0)
        
        # 단어 l의 등장 문서 수 계산
        docs_l <- sum(dtm[, word_l] > 0)
        
        # 첫 번째 식의 로그 비율 추가
        total <- total + log((co_docs + 1) / docs_l)
      }
    }
    coherence[k] <- total
  }
...
semantic_coherence <- CalcCoherence_FirstFormula(topic_word_probs, dtm = dtm_sparse)
...
topic_coherence <- function(topic_model, dtm_data, top_n_tokens = 15,
                                       smoothing_beta = 1){
...
semantic_coherence <- topic_coherence(lda_model, dtm_sparse)
#---------------------------------------------------------------------------#
# Exclusivity 계산
topic_exclusivity <- function(topic_model, top_n_tokens = 15,
                              excl_weight = 0.5){
...
exclusivity <- topic_exclusivity(lda_model)


# 결과 저장
results <- data.frame(
  Topic = 1:length(semantic_coherence),
  Semantic_Coherence = semantic_coherence,
  Exclusivity = exclusivity
)

# 시각화
ggplot(results, aes(x = Semantic_Coherence, y = Exclusivity, label = Topic)) +
  geom_point() +
  geom_text(vjust = -0.5) +
  labs(title = "토픽 품질 평가",
